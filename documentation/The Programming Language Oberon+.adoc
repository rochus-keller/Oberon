...
The most important features of Oberon+ are block structure, modularity, separate compilation, static typing with strong type checking, parametric polymorphism footnote:[adopted with modifications from <<Ro97>>], garbage collection, and type extension with type-bound procedures.
...
Furthermore, enumeration types (known as _scalar types_ in Pascal <<Wi73>>) have been added to the language. IN can be used instead of VAR for constant variable parameters, and procedures can access objects in their environment, also in case of nested procedure declarations.
...
Each character is encoded as a single eight-bit code value using the ISO/IEC 8859-1 encoding scheme.

=== Strings
Strings literals are sequences of characters encoded using the ISO/IEC 8859-1 scheme and enclosed in single (') or double (") quote marks. The opening quote must be the same as the closing quote and must not occur within the string. The number of characters in a string is called its length. A string of length 1 can be used wherever a character constant is allowed and vice versa.
...
[NOTE]
WITH, LOOP and EXIT are Oberon-2 reserved words not present in Oberon-07. DEFINITION and PROC are Oberon+ reserved words not present in previous Oberon versions. All lower-case versions are only reserved words in Oberon+. The compiler is supposed to offer a dedicated Oberon-2 and Oberon-07 compatibility mode to support legacy code with reserved word collisions.
...
The compiler is supposed to offer a dedicated Oberon-2 and Oberon-07 compatibility mode to support legacy code with predeclared identifier collisions.
...
[NOTE]
The precision of BYTE and CHAR is 8 bits. The bit precision of LONGINT is limited by the bit precision of the LONGREAL mantissa bit precision (which is 52 bits in case of IEEE 754 double precision representation). REAL is not expected to (fully) include INTEGER; if an INTEGER is assigned to a REAL, the compiler issues a warning unless the predeclared function FLT is used. The bit precision of SHORTINT is equal or greater than the bit precision of BYTE.
...
=== Procedure types
Variables of a procedure type T have a procedure (or NIL) as value. If a procedure P is assigned to a variable of type T, the formal parameter lists (see <<Formal parameters>>) of P and T must _match_ (see <<Definition of terms>>). P must not be a predeclared or type-bound procedure nor may it be local to another procedure.
...
=== Generics
Oberon+ supports parametric polymorphism. Type and procedure declarations can be made generic by adding formal type parameters. Type parameters are scoped over the declaration and enable it to operate on different types. Generic type and procedure declarations can be instantiated with different types which makes it easier to design reusable algorithms and data structures.

.Syntax:
....
TypeParams       = '<' ident { ',' ident } '>'
TypeActuals      = '<' TypeActual { ',' TypeActual } '>' 
TypeActual       = NamedType | literal
TypeDeclaration  = identdef [TypeParams] '=' type
...
If the receiver of a type bound procedure declaration refers to a parametric type declaration, its actual type parameter must correspond to one of the formal type parameters of the procedure. But it is legal to bind generic procedures to non-generic types.

.Example:
....
proc <T> ( self: List<T> ) print 		// legal
proc <T> ( self: List<integer> ) print  // illegal
....

Instantiated parametric record types do not have subtype relationships induced by their actual type parameters. 
...
Relations yield a BOOLEAN result. The relations `=`, `\#`, `<`, `<=`, `>`, and `>=` apply to the numeric types, CHAR, strings, and character arrays containing `0x` as a terminator. The relations `=` and `#` also apply to BOOLEAN and SET, as well as to pointer and procedure types (including the value NIL). `x IN s` stands for _x is an element of s_. `x` must be of an integer type, and `s` of type SET. `v IS T` stands for _the dynamic type of `v` is T (or an extension of T )_ and is called a type test. It is applicable if
...
=== Procedure calls
A procedure call activates a procedure. It may contain a list of actual parameters which replace the corresponding formal parameters defined in the procedure declaration (see <<Procedure declarations>>). The correspondence is established by the positions of the parameters in the actual and formal parameter lists. There are three kinds of parameters: _variable_ (VAR), IN and _value_ parameters.
...
Let T~f~ be the type of a formal parameter `f` (not an open array) and T~a~ the type of the corresponding actual parameter `a`. For VAR and IN parameters, T~a~ must be the _same type_ as T~f~, or T~f~ must be a record type and T~a~ an _extension_ of T~f~. For value parameters, a must be _assignment compatible_ with `f` (see <<Definition of terms>>).

If T~f~ is an open array, then `a` must be _array compatible_ with `f` (see <<Definition of terms>>). The lengths of `f` are taken from `a`.
...
Globally declared procedures may be associated with a record type declared in the same module. The procedures are said to be bound to the record type. The binding is expressed by the type of the receiver in the heading of a procedure declaration. The receiver may be either a variable (VAR) parameter of record type T or a value parameter of type POINTER TO T (where T is a record type). The procedure is bound to the type T and is considered local to it.
...
If `r` is a receiver parameter declared with type T, `r.P^` denotes the (redefined, sometimes calles _super_) procedure P bound to the base type of T. 
...
[NOTE]
The name of a type-bound procedure must be unique within the type to which it is bound, not within the module in which it is declared.
...
|            |enumeration type     |INTEGER        |ordinal number of the given identifier
...
COPY allows the assignment of a string or a character array containing a terminating 0X to another character array. If necessary, the assigned value is truncated to the target length minus one. The target will always contain 0X as a terminator. 
...
In Oberon+ the import can refer to a module by means of a relative path, where the last ident at the right end of the path corresponds to the imported module name, unless an alias name is defined.
...
Equal types::
    ...
    3. T~a~ and T~b~ are procedure types whose formal parameter lists match. 
...
Type extension (record)::
    ...
    2. T~b~ is a direct extension of an extension of T~a~.
...
Assignment compatible::
    ...
    1. T~e~ and T~v~ are the _same type_;
    2. T~e~ and T~v~ are numeric types and T~v~ _includes_ T~e~;
    3. T~e~ and T~v~ are record types and T~e~ is a _type extension_ of T~v~ and the dynamic type of v is T~v~;
    4. T~e~ and T~v~ are pointer types and T~e~ is a _type extension_ of T~v~;
    5. T~v~ is a pointer or a procedure type and e is NIL;
    6. T~v~ is ARRAY n OF CHAR, e is a string constant with m characters, and m <n;
    7. T~v~ is a procedure type and e is the name of a procedure whose formal parameters match those of T~v~. 

Array compatible::
    An actual parameter a of type T~a~ is array compatible with a formal parameter f of type T~f~ if

    1. T~f~ and T~a~ are the _same type_, or
    2. T~f~ is an open array, T~a~ is any array, and their element types are _array compatible_, or
    3. T~f~ is ARRAY OF CHAR and a is a string. 
...
Matching formal parameter lists::
    ...
    2. they have either the _same function result type_ or none, and
    3. parameters at corresponding positions have _equal types_, and
...
ProcedureDeclaration = ProcedureHeading [ ';' ] 
	ProcedureBody ident 
ProcedureHeading = ( PROCEDURE | PROC ) [TypeParams] [Receiver]
