...
[NOTE]
WITH, LOOP and EXIT are Oberon-2 reserved words not present in Oberon-07. In contrast TRUE and FALSE are Oberon-07 keywords but just predeclared identifiers in Oberon-2 and Oberon+. DEFINITION and PROC are Oberon+ reserved words not present in previous Oberon versions. All lower-case versions are only reserved words in Oberon+. The compiler is supposed to offer a dedicated Oberon-2 and Oberon-07 compatibility mode to support legacy code with reserved word collisions.
...
Relations yield a BOOLEAN result. The relations `=`, `\#`, `<`, `\<=`, `>`, and `>=` apply to the numeric types, as well as CHAR, strings, and character arrays containing `0x` as a terminator. The relations `=` and `#` also apply to BOOLEAN and SET, as well as to pointer and procedure types (including the value NIL). `x IN s` stands for _x is an element of s_. `x` must be of an integer type, and `s` of type SET. `v IS T` stands for _the dynamic type of `v` is T (or an extension of T )_ and is called a type test. It is applicable if
...
Assignment compatible::
    ...
    . T~v~ is a CHAR type and T~e~ is a string constant with one character;
    . T~v~ is a BYTE type and T~e~ is a CHAR or a string constant with one character;
    ...
    . T~v~ is ARRAY n OF CHAR, e is a string constant with m characters, and m < n;
    . T~v~ is ARRAY n OF T, T~e~ is ARRAY m OF T and m <= n, or T~e~ is ARRAY OF T;
    . T~v~ is a procedure type and e is the name of a procedure whose formal parameters match those of T~v~. 
