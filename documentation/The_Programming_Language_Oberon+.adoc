...
The representation of (terminal) symbols in terms of characters is defined using the ISO/IEC 8859-1 character set. Symbols are identifiers, numbers, strings, operators, and delimiters. Source files are encoded with one byte per character. 
...
=== Strings
Strings are sequences of characters encoded using the ISO/IEC 8859-1 scheme and enclosed in single (') or double (") quote marks. The opening quote must be the same as the closing quote and must not occur within the string. The number of characters in a string is called its length. A string of length 1 can be used wherever a character constant is allowed and vice versa.
...
Arrays declared without length are called _open arrays_. They are restricted to pointer base types (see <<Pointer types>>), element types of open array types, and formal parameter types (see <<Formal parameters>>). Arrays declared with length are called _closed arrays_.
...
Equal types::
    ...
    3. T~a~ and T~b~ are procedure types whose formal parameter lists _match_, or
...
Assignment compatible::
    ...
    . T~v~ is a CHAR type and T~e~ is a string with one character;
    . T~v~ is a BYTE type and T~e~ is a CHAR or a string with one character;
    ...
    . T~v~ is ARRAY n OF CHAR and `e` is a string footnote:[which includes single characters corresponding to strings with length 1] with LEN(e) \<= LEN(v);
    . T~v~ and T~e~ are open or closed CHAR arrays, or T~v~ is an open CHAR array and `e` is a string;
...
Array compatible::
    ...
    3. T~f~ is a CHAR array and `a` is a string. 
...
Matching formal parameter lists::
    ...
    4. parameters at corresponding positions are both either value, VAR or IN parameters. 
