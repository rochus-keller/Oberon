...
Procedures bound to a parametric record type don't have explicit type parameters, but use the ones of the record type.

.Example:
....
proc ( self: List ) print 		// legal
proc <T> ( self: List<T> ) print 		// illegal
proc <T> ( self: List<integer> ) print  // illegal
....

The base type of a record can be a parametric record type. Instantiated parametric record types do not have subtype relationships induced by their actual type parameters. 
